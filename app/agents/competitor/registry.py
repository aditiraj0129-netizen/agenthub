...
def add_site(site_id: str, url: str) -> dict:
    with get_db() as conn:
        conn.execute(
            """INSERT INTO competitor_sites (site_id, url, last_report) VALUES (%s, %s, %s)
               ON CONFLICT (site_id) DO UPDATE SET url = EXCLUDED.url, last_report = EXCLUDED.last_report""",
            (site_id, url, None),
        )
...
def update_last_report(site_id: str, report: str):
    with get_db() as conn:
        conn.execute(
            "UPDATE competitor_sites SET last_report = %s WHERE site_id = %s",
            (report, site_id),
        )
...
def delete_site(site_id: str) -> bool:
    with get_db() as conn:
        cur = conn.execute("DELETE FROM competitor_sites WHERE site_id = %s", (site_id,))
