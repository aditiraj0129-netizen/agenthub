        ...
        {pendingEntries.length === 0 ? (
          ...
          <div className="space-y-2">
            {pendingEntries.map(([id, call]) => (
              <div key={id} className="bg-white border border-gray-200 border-l-4 border-l-brand-orange rounded-xl shadow-sm p-4 flex justify-between items-center">
                <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleApprove(id)} className="bg-white border-2 border-green-500 text-green-600 px-3 py-1 rounded-lg text-sm hover:bg-green-500 hover:text-white transition">
                    Approve
                  </button>
                  <button onClick={() => handleReject(id)} className="bg-white border-2 border-red-400 text-red-500 px-3 py-1 rounded-lg text-sm hover:bg-red-400 hover:text-white transition">
                    Reject
                  </button>
                </div>
              </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm divide-y max-h-64 overflow-y-auto">
          ...
          {auditLog.slice().reverse().map((entry, i) => (
            <div key={i} className="p-3 text-sm">
              <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border-2 bg-white mr-2 ${entry.approved === "human" ? "border-brand-purple text-brand-purple" : entry.approved === "rejected" ? "border-red-400 text-red-500" : "border-brand-orange text-brand-orange"}`}>
                {entry.approved}
              </span>
